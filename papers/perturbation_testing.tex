...
decay $0.1$); \code{apply\_constraint} runs after each step. This is the
``sample-and-measure'' paradigm of \cite{bellec2021}: generate freely, then
penalise the dissimilarity between summary statistics of generated and recorded
activity.

\section{Theoretical background and related work}
\label{sec:theory}
This section explains \emph{why} the objective takes the form of
Section~\ref{sec:loss}, and in particular why the generators are trained
free-running rather than autoregressively.

\subsection{Generative distribution matching, not likelihood}
The networks are \emph{generative} models of spontaneous activity: noise is the
only drive, and each \code{generate} call produces statistically independent
trials. The training target is therefore the \emph{distribution} of activity, not
the next spike given the recorded past. The classical alternative --- maximising
the likelihood of the recorded spikes, as generalised linear models (GLMs) do ---
clamps the network's trajectory to the data and optimises one-step-ahead
conditionals. As \cite{bellec2021} showed, such likelihood-fit recurrent spiking
networks ``do not produce realistic neural activity'' when run autonomously. The
sample-and-measure approach instead back-propagates through stochastically
simulated spike trains to match neuroscience summary statistics of the
\emph{freely generated} activity, which is exactly the regime used at test time.

\subsection{Why two complementary losses}
The two terms constrain different statistical moments and neither suffices alone:
\begin{itemize}[leftmargin=1.6em]
\item $\mathcal{L}_{\mathrm{ta}}$ (the PSTH/$\mathcal{L}_{\mathrm{neuron}}$ term)
pins down \emph{each neuron's} trial-averaged response --- mean rate and time
course. But because it averages over trials, a degenerate model that emits the
mean PSTH on every trial (zero variability) scores perfectly; the loss is blind
to trial-to-trial structure. \cite{sourmpis2023} show PSTH-only models cannot
reproduce, e.g., a bimodal single-trial distribution.
\item $\mathcal{L}_{\mathrm{tm}}$ (the trial-matching term) pins down the
\emph{distribution over single trials}. Since generated and recorded trials are
unordered and independent (``no reason for generated trial $k$ to correspond to
recorded trial $k$''), it uses optimal transport: an assignment between the two
trial sets in feature space (Hungarian algorithm on a \emph{detached} cost,
gradient through the matched entries) followed by a matched MSE. But on its own,
the population-level features under-constrain individual neurons.
\end{itemize}
Together they yield correct single-neuron means \emph{and} correct population
trial-to-trial covariability. The eval-time Brain-FID
(Section~\ref{sec:metrics}) is the non-differentiable, distribution-level analogue
of $\mathcal{L}_{\mathrm{tm}}$ on the same feature map.

\subsection{Why free-running, not autoregressive / teacher-forced}
Teacher forcing (feeding the recorded past as input) would be the autoregressive
alternative. It is deliberately avoided here, for four reasons:
\begin{enumerate}[leftmargin=1.8em]
\item \textbf{Exposure bias.} A teacher-forced model conditions on \emph{true}
past states during training but on its \emph{own} states at generation; the
free-running dynamics it must ultimately produce are never directly optimised and
can drift or destabilise. This train--test mismatch is the long-known motivation
for fixes such as Professor Forcing \cite{lamb2016} and scheduled sampling.
Sample-and-measure sidesteps it by optimising the autonomous dynamics directly.
\item \textbf{The variability is the signal.} For spontaneous, noise-driven data
there is no stimulus to ``predict from''. Teacher forcing would let the network
lean on the clamped recorded past instead of learning the recurrent structure
that \emph{generates} the observed variability --- which is precisely the
scientific object of interest.
\item \textbf{Perturbation testing requires autonomy.} The end goal is a
counterfactual: inject the optogenetic current into PV units and predict the
response. In that counterfactual there are \emph{no} recorded spikes to clamp to,
so the model must run autonomously. A teacher-forced model has no validated
free-running behaviour and cannot be used for this prediction.
\item \textbf{Hidden neurons.} Clamping requires observing every neuron;
sample-and-measure handles unobserved/hidden units naturally \cite{bellec2021}.
\end{enumerate}
Note the code still uses back-propagation \emph{within} a trial; it merely
warm-starts each call from the \emph{detached} previous end state
(\code{self.\_state = x.detach()}) and never conditions on recorded spikes, so
there is no gradient coupling across calls and no teacher forcing.

\subsection{Related work}
GLM and maximum-likelihood recurrent spiking networks fit one-step conditionals
with clamped trajectories and are the teacher-forced baseline criticised above.
\cite{bellec2021} introduced the free-running sample-and-measure paradigm;
\cite{sourmpis2023} added the optimal-transport trial-matching loss to capture
variability; the present pipeline follows \cite{sourmpis2026}, which applies these
models to optogenetic perturbation prediction. A different lineage ---
sequential variational auto-encoders such as LFADS \cite{pandarinath2018} ---
uses amortised inference to recover per-trial initial conditions and inferred
inputs and \emph{reconstructs} observed trials; it excels at single-trial
denoising but is not a pure free-running generator matched at the distribution
level, and its inferred inputs complicate clean perturbation counterfactuals.
Distribution-level generative models of spikes (GANs, latent diffusion) share the
``match the distribution'' goal with very different, less mechanistic machinery.

\section{Autoregressive training mode (alternative objective)}
\label{sec:autoregressive}
As a deliberate counterpoint to the free-running, distribution-matching objective
of Section~\ref{sec:loss}, the pipeline also offers an \textbf{autoregressive
roll-out} objective, selected with \code{-{}-train-mode autoregressive} (the
default remains \code{trialmatch}). Here the network state is initialised
\emph{from data} and rolled out open-loop, and the prediction is matched to the
\emph{same} trials --- so trial correspondence is given by construction and no
optimal-transport assignment is needed.

\subsection{Procedure}
For a recorded trial $\mathbf{z}^{\mathcal{D}}$ and a start bin $t_0$, the state
is seeded from the data and the model free-runs for $R$ steps
(\code{-{}-rollout-len}), producing $\hat{\mathbf z}_{t_0:t_0+R}$:
\begin{itemize}[leftmargin=1.6em]
\item \emph{Rate models} (\code{rnn}, \code{lowRNN}): the rate state is set to the
last observed bin, $\xvec_{t_0-1} = \mathbf{z}^{\mathcal{D}}_{t_0-1}$.
\item \emph{Spiking models} (\code{lif}, \code{lowBio}): the delayed-spike buffer
is filled with the last \code{num\_delays} recorded bins and the membrane is
started at $\mathbf{v}=0$ (an approximation; the first steps are a transient).
\end{itemize}
Several start bins $t_0$ are sampled \emph{within} each trial per batch
(\code{-{}-n-inits}), yielding many initial conditions. The roll-out is open-loop
(the network feeds back its own output --- no teacher forcing during the roll-out);
only the initial condition comes from data. As in the free-running mode, gradients
flow within the roll-out but the persistent warm-start buffer is left untouched.

\subsection{Losses}
The trial-matching term $\mathcal{L}_{\mathrm{tm}}$ is dropped. The objective is
\begin{equation}
\mathcal{L} = \lambda_{\mathrm{ta}}\,\mathcal{L}_{\mathrm{ta}}
            + \lambda_{\mathrm{mse}}\,\mathcal{L}_{\mathrm{roll}}
            + \lambda_{\mathrm{band}}\,\mathcal{L}_{\mathrm{band}},
\end{equation}
where (i) $\mathcal{L}_{\mathrm{ta}}$ is the same free-running trial-averaged PSTH
loss as before (it still constrains global per-neuron means); (ii)
$\mathcal{L}_{\mathrm{roll}}$ is the element-wise MSE between the low-pass-filtered
roll-out and the matched data segment,
$\|\,\mathrm{lp}(\hat{\mathbf z}) - \mathrm{lp}(\mathbf z^{\mathcal{D}}_{t_0:t_0+R})\,\|^2$;
and (iii) $\mathcal{L}_{\mathrm{band}}$ is the MSE between the oscillation-band
features $\phi$ of the roll-out and of the \emph{same} data segment --- the same
embedding as $\mathcal{L}_{\mathrm{tm}}$ but with the trials matched by
construction (no Hungarian step). Coefficients are \code{-{}-coeff\_ta},
\code{-{}-coeff\_ar\_mse}, \code{-{}-coeff\_ar\_band}. Because the roll-out is
stochastic, the smoothing keeps $\mathcal{L}_{\mathrm{roll}}$ meaningful only for
the low-frequency, short-horizon component; the band term carries the oscillation
content over the window. Since the features are integrated over the $R$-bin window,
their normalisation statistics are refit on $R$-length crops of the training data
so the lowest bands remain comparable.

\subsection{Effect on metrics and plots}
The free-running generative metrics of Section~\ref{sec:metrics} (PSTH-$r$,
Brain-FID, optimal-transport $R^2$) are still computed each epoch. In addition, a
\textbf{matched roll-out} evaluation is reported: test trials are seeded from data,
rolled out for $R$ steps, and scored against the matched data segment with the
trial order preserved (the $\mathtt{matched}{=}\mathrm{True}$ path), giving a
matched roll-out $R^2$ that becomes the primary tracked quantity and adds a fourth
panel to the over-epochs figure. The training raster switches from free-running
samples to a \emph{matched prediction}: each data trial is shown beside its own
roll-out (data prefix followed by the open-loop continuation). The
perturbation-testing evaluation is \textbf{unchanged} --- it remains free-running
with the injected opto current, since the counterfactual has no spikes to seed
from (Section~\ref{sec:theory}).
...
\code{-{}-train-mode} & \textbf{\code{trialmatch} / \code{autoregressive}} (Section~\ref{sec:autoregressive}) & \code{trialmatch} \\
\code{-{}-rollout-len} & \textbf{roll-out length $R$ in bins (autoregressive)} & $50$ \\
\code{-{}-n-inits} & \textbf{within-trial initial conditions per batch (autoregressive)} & $4$ \\
\code{-{}-coeff\_ar\_mse} & \textbf{weight $\lambda_{\mathrm{mse}}$ of the roll-out MSE (autoregressive)} & $1.0$ \\
\code{-{}-coeff\_ar\_band} & \textbf{weight $\lambda_{\mathrm{band}}$ of the matched-band MSE (autoregressive)} & $0.3$ \\
...
\section{Results}
\label{sec:results}
We report results from the sweep of Section~\ref{sec:settings}, evaluated on the
held-out in-distribution test set (free-running spontaneous activity) and on the
held-out optogenetic perturbation set. All figures are produced directly by the
experiment driver and live under \code{figure\_experiments/}; the per-run metrics
are aggregated in \code{metrics\_summary.csv} (free-running and matched roll-out
scores) and \code{perturbation\_metrics.csv} (per-condition perturbation scores).
Unless noted, the qualitative panels use the best-scoring free-running
configuration, \code{lowBio} with Dale's law, rank $2$ and $\tau_0=2$.

\subsection{Training converges on the free-running metrics}
Figure~\ref{fig:epochs} shows the three free-running metrics over training for a
representative low-rank spiking model. The Brain-FID drops from $\sim\!170$ at
initialisation to below the time-shuffle chance level within a few epochs and
settles near the ideal ceiling (real-train vs.\ real-test), while the
trial-matched $R^2$ rises above chance to $\approx 0.6$. The PSTH Pearson $r$
stays near zero throughout: with noise-driven generators and no stimulus, the
per-neuron trial-averaged trace carries little signal, so the distribution-level
scores (Brain-FID, trial-$R^2$) are the informative ones --- exactly the
motivation for the sample-and-measure objective of Section~\ref{sec:theory}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figure_experiments/freerun/lowBio_dale_rank2_tau2/metrics_over_epochs.png}
\caption{Free-running metrics over training for \code{lowBio}, Dale's law, rank
$2$, $\tau_0=2$. Left: PSTH Pearson $r$; middle: Brain-FID ($\downarrow$); right:
trial-matched $R^2$ ($\uparrow$). Dashed red is the time-shuffle chance level,
dashed green the real-train vs.\ real-test ideal ceiling.}
\label{fig:epochs}
\end{figure}

\subsection{Free-running fit across models}
Table~\ref{tab:freerun} summarises the free-running test scores for one
representative configuration per model family (best $\tau_0$/rank for the low-rank
models). Two patterns are robust across the sweep. First, the area-structured
low-rank models (\code{lowRNN}, \code{lowBio}) reach the lowest Brain-FID and the
highest trial-$R^2$, with the spiking \code{lowBio} best overall (Brain-FID
$\approx 2.8$, trial-$R^2\approx 0.63$), well inside the chance--ceiling band.
Second, the free-running, distribution-matching objective decisively beats the
autoregressive roll-out objective on these distribution-level scores: in
autoregressive mode the free-running Brain-FID and trial-$R^2$ collapse (most
configurations score negative trial-$R^2$; cf.\ the \code{ar} rows of
\code{metrics\_summary.csv}), consistent with the exposure-bias argument of
Section~\ref{sec:theory}. The matched roll-out $R^2$ that the autoregressive mode
optimises stays modest ($\lesssim 0.16$), i.e.\ open-loop prediction of single
spontaneous trials remains hard even when trials are matched by construction.
...
\begin{tabular}{@{}llccc@{}}
\toprule
Model & Constraint & rank,\,$\tau_0$ & Brain-FID $\downarrow$ & trial-$R^2$ $\uparrow$ \\
\midrule
\code{rnn}    & unconstrained & ---       & $8.41$  & $0.458$ \\
\code{rnn}    & Dale          & ---       & $14.71$ & $0.165$ \\
\code{lif}    & unconstrained & ---       & $8.94$  & $0.355$ \\
\code{lif}    & Dale          & ---       & $6.92$  & $0.501$ \\
\code{lowRNN} & Dale          & $2,\,2$   & $4.52$  & $0.609$ \\
\code{lowBio} & Dale          & $2,\,2$   & $\mathbf{2.79}$  & $\mathbf{0.630}$ \\
\bottomrule
\end{tabular}
\caption{Free-running test metrics (\code{freerun} mode) for representative
configurations. Lower Brain-FID and higher trial-$R^2$ are better. The
area-structured low-rank models dominate, and the spiking \code{lowBio} is best.}
\label{tab:freerun}
\end{table}

\subsection{Perturbation testing}
The decisive test is the held-out optogenetic perturbation, which no model saw
during training. Figures~\ref{fig:pert-fid} and~\ref{fig:pert-r2} sweep the
raised-cosine perturbation: each panel plots a metric against the recorded light
intensity ($1.3$, $1.7$, $2.0$), with one row per training mode
(\code{freerun}/\code{ar}) and one column per opto-injection scale
(\code{-{}-opto-intensities} $\in\{0.01,0.1,1.0\}$); lines are means over rank and
$\tau_0$ per model family, and the dashed traces are the no-perturbation baseline.
Performance degrades markedly relative to the free-running fit: Brain-FID rises
well above its unperturbed value and trial-$R^2$ frequently goes negative, i.e.\
predicting the causal effect of PV stimulation is substantially harder than
matching spontaneous statistics --- the expected and scientifically interesting
outcome of perturbation testing. The per-run heatmap of Figure~\ref{fig:heatmap}
shows this is consistent across the full sweep (columns are opto-scale\,$\times$
light-strength), with the low-rank spiking models again the least degraded.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figure_experiments/cosine_brain_fid.png}
\caption{Raised-cosine perturbation, Brain-FID ($\downarrow$) vs.\ light strength.
Rows: training mode (\code{freerun}, top; \code{ar}, bottom). Columns: opto
injection scale. Solid lines are model families (mean over rank/$\tau_0$); dashed
lines are the no-perturbation baseline.}
\label{fig:pert-fid}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figure_experiments/cosine_trial_r2.png}
\caption{Raised-cosine perturbation, trial-matched $R^2$ ($\uparrow$) vs.\ light
strength, same layout as Figure~\ref{fig:pert-fid}. Values frequently fall below
zero, quantifying how much harder the held-out perturbation is than the
free-running fit.}
\label{fig:pert-r2}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figure_experiments/cosine_fid_heatmap.png}
\caption{Per-run Brain-FID under the raised-cosine perturbation (log colour
scale). Rows are individual runs (model\,$\times$\,constraint\,$\times$\,rank\,$\times\,\tau_0$),
columns are opto-scale\,$\times$\,light-strength; \code{freerun} (left) vs.\
\code{ar} (right).}
\label{fig:heatmap}
\end{figure}

\subsection{Qualitative perturbation response}
Figure~\ref{fig:pert-raster} shows ground-truth (left) vs.\ generated (right)
rasters for the strongest raised-cosine condition. The model's trial-averaged
population activity (top panels) clearly tracks the injected raised-cosine light
waveform, so the network does produce a perturbation-locked response of roughly
the right gross shape; but the single-trial structure (lower panels) and the
amplitude/timing details diverge from the data, which is why the quantitative
trial-$R^2$ for this condition is poor despite the qualitatively sensible drive.
This gap between a plausible mean response and a correct single-trial
distribution is precisely what the distribution-level perturbation metrics are
designed to expose.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{../figure_experiments/freerun/lowBio_dale_rank2_tau2/opto_1.00/perturbation_raised_cosine_l2.0.png}
\caption{Qualitative perturbation prediction (\code{lowBio}, Dale, rank $2$,
$\tau_0=2$, opto scale $1.0$, raised-cosine $l=2.0$). Left: recorded; right:
generated. Top row: trial-averaged population activity (blue) overlaid on the
injected light waveform (orange); lower rows: example single trials with per-area
rasters. The generated mean follows the light, while single-trial details
diverge.}
\label{fig:pert-raster}
\end{figure}
...
\bibitem{bellec2021}
G.\ Bellec, S.\ Wang, A.\ Modirshanechi, J.\ Brea, and W.\ Gerstner.
\newblock Fitting summary statistics of neural data with a differentiable spiking
network simulator.
\newblock \emph{Advances in Neural Information Processing Systems (NeurIPS)}, 2021.
\newblock \url{https://arxiv.org/abs/2106.10064}.

\bibitem{pandarinath2018}
C.\ Pandarinath et al.
\newblock Inferring single-trial neural population dynamics using sequential
auto-encoders (LFADS).
\newblock \emph{Nature Methods}, 15:805--815, 2018.
\newblock \url{https://www.nature.com/articles/s41592-018-0109-9}.
...
\newblock \url{https://arxiv.org/abs/1610.09038}.
\end{thebibliography}
